...
(begin
      ...
      (letrec ((non-terminals
		(lambda (grammar)
		  (letrec ((add-nt (lambda (nt nts) (if (member nt nts) nts (cons nt nts)))))
	       ...
	       (ind
		(lambda (nt nts)
		  ((letrec ((loop (lambda (i) (if (>= i '0) (if (equal? (vector-ref nts i) nt) i (loop (- i '1))) '#f))))
		     loop)
		   (- (vector-length nts) '1))))
	       (nb-configurations
		(lambda (grammar)
						      ...
						      ((letrec ((loop1
								 (lambda (l)
								   (if (pair? l)
								       (let ((starter (car l)))
									 (let ((starter-set
										(conf-set-get* state state-num starter)))
								 ...
								 (lambda (l)
								   (if (pair? l)
									 ...
									 (if (conf-set-member? state ender state-num)
									     (let ((next (+ conf '1)))
									       (let ((next-set
										      (conf-set-get* state state-num next)))
								   ...
								   (let ((conf-set (vector-ref state (+ conf '1))))
								     (let ((head (vector-ref conf-set '4)))
									   ...
									   (predict
									    ...
									    enders)
									   (let ((preds
										  (vector-ref predictors (+ step nb-nts))))
									     (reduce
									      states
									      state
									      state-num
  ...
  (define test
	...
	(let ((x (p (vector->list (make-vector k 'a))))) (display (length (parse->trees x 's '0 k))) (newline)))))
  (test '12))
