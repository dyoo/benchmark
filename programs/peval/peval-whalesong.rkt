        ...
        (cond ((const-expr? exp))  ; leave constants the way they are
              ...
              ((eq? (car exp) 'begin)
               ...
               (let loop ((exps exp)) ; remove all useless expressions
                 (when (not (null? (cddr exps))) ; not last expression?
                   (let ((x (cadr exps)))
                     (loop (if (or (const-expr? x)
                             ...
                             (cdr exps))))))
               (when (null? (cddr exp)) ; only one expression in the begin?
    ...
    (define (rec! where)
      (let ((exp (car where)))
        ...
        (cond ((const-expr? exp))
              ...
              (else
               (for-each! rec! (cdr exp))
               (when (and (null? (cdr exp)) (symbol? (car exp)))
                 (let ((frame (binding-frame (car exp) env)))
                   (when frame ; is it a bound variable?
                     (let ((proc (bound-expr (car exp) frame)))
                       (when (and (pair? proc)
                                (eq? (car proc) 'lambda))
                         (begin
    ...
    (define (rc exp ae)
      (cond ((const-expr? exp))
            ...
            (else
             ...
             (when (symbol? (car exp))
               (when (eq? (car exp) var) (set! oper (+ oper 1)))))))
    (rc exp #t)
