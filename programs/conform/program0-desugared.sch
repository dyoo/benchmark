                          ...
                          (lambda (object selector mutator)
                            (mutator
                             ...
                             (map
                              ...
                              (selector object))))))
                  (begin
                    (if (not (let ((or-part (none-node? node))) 
                               (if or-part or-part (any-node? node))))
                        (begin
  ...
  (lambda (t1 t2)
    (letrec ((nodes-with-red-edges-out (box '()))
      ...
      (let ((result (does-conform t1 t2)))
        (begin (for-each (if result greenify-red-edges! delete-red-edges!) (unbox nodes-with-red-edges-out))
               result)))))
...
(define test
  (lambda () 
    (begin
      (reset) 
      (map name (graph-nodes 
                 (make-lattice 
                  (make-graph 
                   (list (unbox a) (unbox b) (unbox c) (unbox d) any-node none-node))
                  '#t))))))
